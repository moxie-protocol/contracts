...
const getTokenLockWalletOrFail = async (hre: HardhatRuntimeEnvironment, address: string) => {
  const wallet = await hre.ethers.getContractAt('MoxieTokenLockWallet', address)
  try {
    await wallet.deployed()
  } catch (err) {
    logger.error('MoxieTokenLockWallet not deployed at', wallet.address)
    process.exit(1)
  }
...
task('cancel-token-lock', 'Cancel token lock contract')
  ...
  .setAction(async (taskArgs, hre: HardhatRuntimeEnvironment) => {
    ...
    logger.log(`> MoxieTokenLockWallet: ${lockWallet.address}`)
