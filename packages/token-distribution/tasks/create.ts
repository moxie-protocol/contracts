...
const prettyConfigEntry = (config: TokenLockConfigEntry) => {
  return `
    ...
    Amount: ${formatEther(config.managedAmount)} MOXIE
...
const calculateSalt = async (
  ...
  const factory = await getContractFactory(hre, 'MoxieTokenLockWallet')
...
export const getTokenLockManagerOrFail = async (hre: HardhatRuntimeEnvironment, name: string) => {
  ...
  if (!deployment.address) {
    logger.error('MoxieTokenLockManager address not found')
    process.exit(1)
  }

  const manager = await hre.ethers.getContractAt('MoxieTokenLockManager', deployment.address)
  try {
    await manager.deployed()
  } catch (err) {
    logger.error('MoxieTokenLockManager not deployed at', manager.address)
    process.exit(1)
  }
...
task('create-token-locks', 'Create token lock contracts from file')
  ...
  .setAction(async (taskArgs, hre: HardhatRuntimeEnvironment) => {
    ...
    if (!taskArgs.txBuilder) {
      ...
      const currentBalance = await MOXIE.balanceOf(manager.address)
      logger.log(`> Amount to distribute:  ${formatEther(totalAmount)} MOXIE`)
      logger.log(`> Amount in the Manager: ${formatEther(currentBalance)} MOXIE`)
      if (currentBalance.lt(totalAmount)) {
        logger.error(`MoxieTokenLockManager is underfunded. Deposit more funds into ${manager.address}`)
        process.exit(1)
      }
    ...
    if (!taskArgs.txBuilder) {
      ...
      if (currentBalance.lt(totalAmount)) {
        ...
        // Use MOXIE.approve + the manager deposit function instead of MOXIE.transfer to be super safe
        const approveTx = await MOXIE.populateTransaction.approve(manager.address, remainingBalance)
        txBuilder.addTx({
          to: tokenAddress,
          value: '0',
          data: approveTx.data,
...
task('create-token-locks-simple', 'Create token lock contracts from file')
  ...
  .setAction(async (taskArgs, hre: HardhatRuntimeEnvironment) => {
    ...
    for (const entry of entries) {
      ...
      try {
        const tokenLockSimpleFactory = await getContractFactory(hre, 'MoxieTokenLockSimple')
...
task('scan-token-locks-balances', 'Check current balances of deployed contracts')
  ...
  .setAction(async (taskArgs, hre: HardhatRuntimeEnvironment) => {
    ...
    for (const entry of deployedEntries) {
      balances = balances.add(await MOXIE.balanceOf(entry.contractAddress))
    }
