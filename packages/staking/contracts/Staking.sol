...
contract Staking is IStaking,SecurityModule, ReentrancyGuard , OwnableUpgradeable {
    ...
    constructor(address _tokenManager, address _moxieBondingCurve,address _moxieToken, uint256 _lockPeriod) {
    ...
    */
     ...
     * @param amount AMount of tokens getting deposited.
     */
    function _deposit(address _subject, uint256 amount) internal {
        if (amount == 0) {
            revert AmountShouldBeGreaterThanZero();
        }
        IERC20Extended subjectToken = IERC20Extended(i_tokenManager.tokens(_subject));
        ...
        // emit event
        emit Deposit(msg.sender, _subject, address(subjectToken), index, amount, unlockTime);
        // Transfer the tokens to this contract
        bool success = subjectToken.transferFrom(msg.sender, address(this), amount);
        if (!success) {
     ...
     */
    function depositAndLock(address _subject, uint256 _amount) external nonReentrant {
    ...
    function buyAndLock(address _subject, uint256 _depositAmount, uint256 _minReturnAmountAfterFee)
        external
        nonReentrant
    {
        // transfer moxie to this contract
        i_moxieToken.transferFrom(msg.sender, address(this), _depositAmount);
        // pull moxie
        uint256 _amount = i_moxieBondingCurve.buyShares(_subject, _depositAmount, _minReturnAmountAfterFee);
        uint256 unlockTime = block.timestamp + s_lockPeriod;
        ...
        uint256 index = s_LockCount++;
        s_locks[index] = lockInfo;
        emit Deposit(msg.sender, _subject, subjectToken, index, _amount, unlockTime);
    }
    ...
    function withdraw(uint256[] memory _indexes) external nonReentrant {
        ...
        IERC20Extended subjectToken = IERC20Extended(firstLockInfo.subjectToken);
        for (uint256 i = 0; i < _indexes.length; i++) {
            LockInfo memory lockInfo = s_locks[_indexes[i]];
            if (firstLockInfo.subject != lockInfo.subject) {
                revert SubjectsDoesntMatch();
            }
        ...
        emit Withdraw(msg.sender, firstLockInfo.subject, firstLockInfo.subjectToken, _indexes, totalAmount);
    }

    function getLockInfo(uint256 _index) external view returns (LockInfo memory) {
