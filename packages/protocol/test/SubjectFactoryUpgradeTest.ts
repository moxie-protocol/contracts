...
describe("SubjectFactoryUpgradeTest", () => {
  const deploy = async () => {
    const [
      deployer,
      owner,
    ...
    const SubjectFactoryLegacyV1 =
      await hre.ethers.getContractFactory("SubjectFactory");
    const SubjectFactory =
      await hre.ethers.getContractFactory("SubjectFactoryV2");
    ...
    const EasyAuction =
      await hre.ethers.getContractFactoryFromArtifact(EasyAuctionArtifact);
    ...
    const easyAuction: EasyAuction = (await EasyAuction.connect(
      owner,
  ...
  describe("deployment", () => {
    it("verify deployment", async () => {
      ...
      expect(
        await subjectFactory.hasRole(
          await subjectFactory.DEFAULT_ADMIN_ROLE(),
          owner.address,
        ),
      ).to.true;
      ...
      expect(await subjectFactory.moxieBondingCurve()).to.equal(
        moxieBondingCurveAddress,
      );
      expect(await subjectFactory.token()).to.equal(moxieTokenAddress);
      expect(await subjectFactory.subjectFeePct()).equal(
        feeInputSubjectFactory.subjectFeePct,
      );
      expect(await subjectFactory.protocolFeePct()).equal(
        feeInputSubjectFactory.protocolFeePct,
      );
      expect(await subjectFactory.feeBeneficiary()).equal(
        feeBeneficiary.address,
      );
      expect(await subjectFactory.auctionDuration()).equal(auctionDuration);
      expect(await subjectFactory.auctionOrderCancellationDuration()).equal(
        auctionCancellationDuration,
      );
    });
  ...
  describe("initiate subject onboarding", () => {
    it("should initiate Subject Onboarding", async () => {
      ...
      expect(await subjectToken.balanceOf(easyAuctionAddress)).to.equal(
        auctionInput.initialSupply,
      );
    ...
    it("should fail to initiate for zero subject address", async () => {
      const { subjectFactory, owner, moxiePassVerifierAddress } =
        await loadFixture(deploy);
    ...
    it("should fail to initiate Subject when contract is paused", async () => {
      ...
      await subjectFactory
        .connect(owner)
        .grantRole(await subjectFactory.PAUSE_ROLE(), owner.address);
  ...
  describe("update fee", () => {
    it("should be able to update fee", async () => {
      ...
      await subjectFactory
        .connect(owner)
        .grantRole(await subjectFactory.UPDATE_FEES_ROLE(), deployer.address);
      ...
      await expect(subjectFactory.connect(deployer).updateFees(feeInput))
        ...
        .withArgs(feeInput.protocolFeePct, feeInput.subjectFeePct);
    });
  ...
  describe("update AuctionTime", () => {
    it("should  be able to update auction time with permissions", async () => {
      ...
      await expect(
        await subjectFactory
          .connect(deployer)
          .updateAuctionTime(auctionDuration, auctionOrderCancellationDuration),
      )
        .to.emit(subjectFactory, "UpdateAuctionParam")
        .withArgs(auctionDuration, auctionOrderCancellationDuration);
    });
    ...
    it("should not be able to update auction time without permissions", async () => {
      ...
      await expect(
        ...
        .withArgs(deployer.address, await subjectFactory.UPDATE_AUCTION_ROLE());
    });
  ...
  describe("finalize subject onboarding", () => {
    it("should finalize subject onboarding when there are bids in auction", async () => {
      ...
      await moxieToken
        .connect(bidder1)
        .approve(easyAuctionAddress, biddingAmount);
      ...
      const additionalSupplyDueToBuyAmount =
        (BigInt(buyAmount) * BigInt(auctionInput.initialSupply)) /
        BigInt(biddingAmount);

      const expectedProtocolFee =
        ((BigInt(biddingAmount) + BigInt(buyAmount)) *
    ...
    it("should finalize subject onboarding when there are no bids in auction", async () => {
      ...
      const additionalSupplyDueToBuyAmount =
        (BigInt(buyAmount) * BigInt(auctionInput.initialSupply)) /
        BigInt(auctionInput.minBuyAmount);

      const expectedProtocolFee =
        (BigInt(buyAmount) * BigInt(feeInputSubjectFactory.protocolFeePct)) /
        PCT_BASE;
      const expectedSubjectFee =
        (BigInt(buyAmount) * BigInt(feeInputSubjectFactory.subjectFeePct)) /
        PCT_BASE;
      const expectedBondingSupply = additionalSupplyDueToBuyAmount;
      const expectedBondingAmount =
        BigInt(buyAmount) - expectedProtocolFee - expectedSubjectFee;
    ...
    it("should finalize subject onboarding when there are  bids for half of initial auction amout", async () => {
      ...
      const expectedSubjectFee =
        ...
        PCT_BASE;
      const expectedBondingSupply =
        (BigInt(biddingAmount) * BigInt(auctionInput.initialSupply)) /
          BigInt(auctionInput.minBuyAmount) +
        additionalSupplyDueToBuyAmount;
      const expectedBondingAmount =
        BigInt(biddingAmount) +
        BigInt(buyAmount) -
        expectedProtocolFee -
      ...
      const subjectFeeOnSubjectBuyAmount =
        (expectedSubjectFee * BigInt(feeInput.subjectBuyFeePct)) / PCT_BASE;
      const protocolFeeOnSubjectBuyAmount =
        (expectedSubjectFee * BigInt(feeInput.protocolBuyFeePct)) / PCT_BASE;

      const actualBuyAmountFromSubjectFee =
        expectedSubjectFee -
        subjectFeeOnSubjectBuyAmount -
        protocolFeeOnSubjectBuyAmount;
    ...
    it("should not finalize subject onboarding when caller doesnot have buy amount", async () => {
      ...
      await moxieToken
        .connect(owner)
        .burn(await moxieToken.balanceOf(owner.address));
    ...
    it("should not finalize subject onboarding when reserve ratio is zero", async () => {
      ...
      await expect(
        subjectFactory
          .connect(owner)
          .finalizeSubjectOnboarding(subject.address, buyAmount, 0),
      ).to.revertedWithCustomError(
        subjectFactory,
    ...
    it("should fail when auction doesnot exists", async () => {
      ...
      await expect(
        subjectFactory
          .connect(owner)
          .finalizeSubjectOnboarding(owner.address, buyAmount, reserveRatio),
      ).to.revertedWithCustomError(
        subjectFactory,
    ...
    it("should not finalize subject onboarding when caller doesnot have onboarding role", async () => {
      ...
      await subjectFactory
        .connect(owner)
        .revokeRole(await subjectFactory.ONBOARDING_ROLE(), owner.address);
