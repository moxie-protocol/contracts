...
const whoHasMoxie = async () => {
  console.log(
    "this script is to check who has moxie, which can be used to buy & lock later",
  );
  ...
  const balance = await moxieToken.balanceOf(
    "0xb6ca234e277656bc8a9cff1d30cd4ccefa32d7a5",
  );

  console.log(`Token lock wallet has ${balance.toString()} moxie`);
};
