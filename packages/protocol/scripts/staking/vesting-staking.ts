...
const main = async () => {
 ...
 const moxieTokenLockManager = await ethers.getContractAt("IMoxieTokenLockManager", MOXIE_TOKEN_LOCK_MANAGER)
 await moxieTokenLockManager.connect(owner).addSubjectTokenDestination(STAKING)
 await moxieTokenLockManager.connect(owner).addTokenDestination(STAKING)

 let addresses = Array(4).fill(STAKING)
 await moxieTokenLockManager.connect(owner).setAuthFunctionCallMany(sigs, addresses)
}
