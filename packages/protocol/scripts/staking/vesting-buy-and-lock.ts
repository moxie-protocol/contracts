...
// 2. buyAndLock
export const getBuyAndLockCalldata = (
  _subject: string,
  _depositAmount: string,
  _minReturnAmountAfterFee: string,
  _lockPeriod: string,
): string => {
  const abi = [
  ...
  const iface = new Interface(abi);
  return iface.encodeFunctionData("buyAndLock", [
    _subject,
    _depositAmount,
...
// 3. withdraw
export const getWithdrawCalldata = (
  _indexes: string[],
  _subject: string,
): string => {
  const abi = [
...
// 4. extendLock
export const getExtendLockCalldata = (
  _indexes: string[],
  _subject: string,
  _lockPeriod: string,
): string => {
  ...
  const iface = new Interface(abi);
  return iface.encodeFunctionData("extendLock", [
    _indexes,
    _subject,
    _lockPeriod,
  ]);
