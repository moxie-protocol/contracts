...
export default buildModule("UpgradePermissions", (m) => {
  ...
  const {  moxieBondingCurveV2Instance, subjectFactoryV2Instance,  } = m.useModule(UpgradeProtocol)
  const { protocolRewardInstance } = m.useModule(ProtocolRewards);

  // set UPDATE_PROTOCOL_REWARD_ROLE role
  const updateProtocolRewardRole = m.staticCall(moxieBondingCurveV2Instance, "UPDATE_PROTOCOL_REWARD_ROLE");
  const updateFeesRole = m.staticCall(moxieBondingCurveV2Instance, "UPDATE_FEES_ROLE");
  // TODO: verify who should be the beneficiary
  const grantUpdateProtocolRewardRole = m.call(moxieBondingCurveV2Instance, 'grantRole',
    [updateProtocolRewardRole, owner],
    { from: owner, id: "moxieBondingCurveV2UpdateProtocolRewardRole" }
  );
  const grantUpdateFeesRole = m.call(moxieBondingCurveV2Instance, 'grantRole',
    [updateFeesRole, owner],
    { from: owner, id: "moxieBondingCurveV2UpdateFeesRole" }
  );
  // set the protocol rewARD contract 
  m.call(moxieBondingCurveV2Instance, "updateProtocolRewardAddress",
  ...
  m.call(moxieBondingCurveV2Instance, "updateReferralFee",
  ...
  const updateProtocolRewardRoleSubjectFactory = m.staticCall(subjectFactoryV2Instance, "UPDATE_PROTOCOL_REWARD_ROLE");
  const updateFeesRoleSubjectFactory = m.staticCall(subjectFactoryV2Instance, "UPDATE_FEES_ROLE");


  const grantUpdateProtocolRewardRoleSubjectFactory = m.call(subjectFactoryV2Instance, 'grantRole',
    [updateProtocolRewardRoleSubjectFactory, owner],
    { from: owner, id: "subjectFactoryV2UpdateProtocolRewardRole" }
  );
  
  const grantUpdateFeesRoleSubjectFactory = m.call(subjectFactoryV2Instance, 'grantRole',
    [updateFeesRoleSubjectFactory, owner],
    { from: owner, id: "subjectFactoryV2UpdateFeesRole" }
  );

  m.call(subjectFactoryV2Instance, "updateProtocolRewardAddress",
    [protocolRewardInstance],
    {
      from: owner,
    ...
    m.call(subjectFactoryV2Instance, "updatePlatformReferrerFee",
